...
class FirstPage:
    def __init__(self, main_window):
        ...
        def filter(letter):
            ...
            pi_details = list(product_info.values())
            word = []
            letter_matched = []
            amount_matched = []
            #lm = 0
            for each_word in range(len(product_info_name)):
                ...
                for check_letter in list(word[each_word]):
                    if check_letter[0] == letter:
                        amount_matched.append(product_info_name[each_word])
                        # lm += 1
                    else:
                        break
            ##--- Main Function ---##
            #print(f"{lm} is matching '{letter}'")

            max_frame_per_page = 25
            max_num_of_column = 5  # Maximum number of frames in one row (This is a constant value)
            num_of_frame_column = len(amount_matched)  # Number of Lists in the main list (This value can be changed)
            ...
            def load_page(num_of_frame_column, current_rows, page_, current_value_row):
                    ...
                    for product_rows in range(len(current_columns[current_page])):
                        ...
                        for product_columns in range(current_columns[current_page][product_rows]):
                            product_frame = customtkinter.CTkFrame(home_frame, width=240, height=300, fg_color="#949494",
                                                                   corner_radius=3)
                            product_frame.grid(row=0 + product_rows, column=0 + product_columns, sticky="W", padx=23,
                                               pady=23)
                        ...
                        if (current_page >= 0 and (total_display_frames > max_frame_per_page)
                            ...
                            next_page_btn = customtkinter.CTkButton(home_frame, text='>',
                ...
                for each_word2 in range(len(product_info_name)):
                    word.append(product_info_name[each_word2])
                    for check_letter2 in list(word[each_word2]):
                        if check_letter2[0] == letter:
                            letter_matched[current_page].append([product_info_name[each_word2], pi_details[each_word2]])
                        else:
                            break
                ...
                try:
                    for create_lbl in range(len(letter_matched[current_page])):  # original: len(product_info_name)
                        ## --- Product Image --- ##
                        # Original list used for display: product_info_details[create_lbl][1][2]
                        try:
                            product_image = Image.open(f'{letter_matched[current_page][create_lbl][1][1][2]}')
                            ...
                            product_image_display = customtkinter.CTkButton(
                                ...
                                cursor="hand2",
                                command=lambda
                                    title=letter_matched[current_page][create_lbl][0],
                                    price=letter_matched[current_page][create_lbl][1][0][0],
                                    image=letter_matched[current_page][create_lbl][1][1][2],
                                    description=letter_matched[current_page][create_lbl][1][0][1],
                                    uuid=letter_matched[current_page][create_lbl][1][1][3],
                                    stocks=letter_matched[current_page][create_lbl][1][0][2]:
                            ...
                            product_title = customtkinter.CTkLabel(
                                product_frame_list[change_config_row][change_parent_frame],
                                text=letter_matched[current_page][create_lbl][0], font=("Bold", 16))
                            ...
                            price_lbl = customtkinter.CTkLabel(product_frame_list[change_config_row][change_parent_frame],
                                                               text=f"${letter_matched[current_page][create_lbl][1][0][0]}",
                                                               font=("Bold", 17))
                            ...
                            product_info_widgets[f"product_frame{create_lbl + 1}"] = [
                                pi_information_[current_page][create_lbl][1][1][2],
                                product_title,
                                price_lbl,
                                add_to_cart_btn,
                                "",
        ...
        deposit_credit = customtkinter.CTkEntry(animated_panel2, font=("Century Gothic", 20, "bold"))
        deposit_credit.place(relx=0.125, rely=0.19)

        deposit_text = customtkinter.CTkLabel(animated_panel2, text="", font=("Century Gothic", 20, "bold"),
                                                text_color="#313131")
        deposit_text.place(relx=0.125, rely=0.23)

        def deposit_credits():
            global Credit

            if deposit_credit.get() == str(0):
                deposit_text.configure(text="Fail to deposit, please enter an amount!",
                                        font=("Century Gothic", 16, "bold"), text_color="#ff1919")
                deposit_text.after(5000, lambda: deposit_text.configure(text=""))
            try:
                if int(deposit_credit.get()) > 10000:  #Max credit allow for deposit, in this case its: $10,000
                    deposit_text.configure(text="Fail to deposit, exceeded maximum amount(1 - 10,000)!",
                                            font=("Century Gothic", 16, "bold"), text_color="#ff1919")
                    deposit_text.after(5000, lambda: deposit_text.configure(text=""))

                if 1 <= int(deposit_credit.get()) <= 10000:
                    Credit += int(deposit_credit.get())
                    display_credit.configure(text=f"Credit: ${Credit:,}")
            except:
                deposit_text.configure(text="Fail to deposit, please enter integers/floats only!",
                                       font=("Century Gothic", 16, "bold"), text_color="#ff1919")
                deposit_text.after(5000, lambda: deposit_text.configure(text=""))

            deposit_credit.delete(0, "end")


        deposit_btn = customtkinter.CTkButton(animated_panel2, text="Deposit", font=("Century Gothic", 20, "bold"),
                                              text_color="#313131", command=lambda: deposit_credits())
        deposit_btn.place(relx=0.125, rely=0.27)
        ...
        def product_details_page(product_title, product_price, product_image, product_description, product_uuid, qty,
                                 cframe):
            hide_indicate()
            try:
                clear_frame(home_frame)
            ...
            pd_description = customtkinter.CTkLabel(pd_frame, text="------------[ Description ]------------",
                                                    ...
                                                    text_color="#313131")
            pd_description.place(relx=0.615, rely=0.3275, anchor=CENTER)

            pd_description = customtkinter.CTkLabel(pd_frame, text=f"{product_description}",
        ...
        def home_page(current_page):
            ...
            def load_page(num_of_frame_column, current_rows, page_, current_value_row):
                ...
                # Creating a multidimensional list to store other list that are represented as individual frames
                try:
                    for i in range(len(product_info_name)):   #sum(current_columns[pg])
                        if i < len(product_info_name):
                            if frames_added >= max_frame_per_page:  # len(pi_information_[pg])
                                pg += 1
                                frames_added = 0
                            pi_information_[pg][frames_added].append(product_info_name[i])
                            pi_information_[pg][frames_added].append(product_info_details[i])
                            frames_added += 1
                        """else:
                            break"""
                except:
                ...
                for create_lbl in range(len(pi_information_[current_page])):  # original: len(product_info_name)
                    ...
                    try:
                        ...
                        product_title = customtkinter.CTkLabel(
                            product_frame_list[change_config_row][change_parent_frame],
                            text=pi_information_[current_page][create_lbl][0], font=("Bold", 16))
                        product_title.place(relx=0.5, rely=0.875, anchor=CENTER)
        ...
        def search_page(current_page):
            ...
            def load_page(num_of_frame_column, current_rows, page_, current_value_row):
                ...
                for product_rows in range(len(current_columns[current_page])):
                    ...
                    if (current_page >= 0 and (total_display_frames > max_frame_per_page)
                            and (sum(current_columns[current_page]) == max_frame_per_page)):
                        next_page_btn = customtkinter.CTkButton(search_frame, text='>',
                                                                font=("Century Gothic", 22, "bold"),
                                                                cursor="hand2", width=35, height=15,
                                                                command=lambda: search_page(current_page + 1),
                                                                fg_color="#c5c5c5", hover_color="#a8cce8",
                                                                text_color="#313131")
                        next_page_btn.grid(row=6, column=3, sticky="S", pady=40)
                    ...
                    if current_page > 0:
                        if len(current_columns[current_page]) < max_num_of_column:
                            px = 120
                        back_page_btn = customtkinter.CTkButton(search_frame, text='<',
                                                                font=("Century Gothic", 22, "bold"),
                                                                cursor="hand2", width=35, height=15,
                                                                command=lambda: search_page(current_page - 1),
                                                                fg_color="#c5c5c5", hover_color="#a8cce8",
                    ...
                    indicator_pages = customtkinter.CTkLabel(search_frame,
                                                             text=f'{current_page + 1}/{len(current_columns)}',
                                                             font=("Century Gothic", 22, "bold"),
                                                             text_color="#313131")
                    indicator_pages.grid(row=6, column=2, sticky="S", pady=40, padx=px)
                ...
                productw_atc3 = list(product_info_widgets.values())

                for pw_atc3 in range(len(productw_atc3)):
                    OnHoverATC3(productw_atc3[pw_atc3][3], "#ffd3a4", "#949494",
                               "#ff931c", "#7b7b7b")
                ...
                try:
                    for create_lbl in range(len(pi_information_[current_page])):  # original: len(product_info_name)
                        ...
                        try:
                            ...
                            product_title = customtkinter.CTkLabel(
                                product_frame_list[change_config_row][change_parent_frame],
                                text=new_pi[current_page][create_lbl][0], font=("Bold", 16))
                            product_title.place(relx=0.5, rely=0.875, anchor=CENTER)
                            ...
                            add_to_cart_btn = customtkinter.CTkButton(
                                ...
                                command=lambda name=product_title.cget('text'),
                                               pi_index=create_lbl:
                                add_to_cart(name, new_pi[current_page][pi_index][1][0][0],
                                            new_pi[current_page][create_lbl][1][1][3], 1))
                            add_to_cart_btn.place(relx=0.775, rely=0.75, anchor=CENTER)

                            ## --- Store all 4 widgets to a dictionary --- ##
                            product_info_widgets[f"product_frame{create_lbl + 1}"] = [
                                new_pi[current_page][create_lbl][1][1][2],
                                product_title,
                                price_lbl,
                                add_to_cart_btn,
                                "",
                                new_pi[current_page][create_lbl][1][0][1],
                                new_pi[current_page][create_lbl][1][1][3],
                                new_pi[current_page][create_lbl][1][0][2]]
            ...
            def checkout():
                ...
                if float(total_cost) > float(Credit):
                    validity_text.configure(text="Checkout invalid, you are poor!",
                                            font=("Century Gothic", 16, "bold"), text_color="#ff1919")
                    validity_text.after(5000, lambda: validity_text.configure(text=""))
                if len(Cart_Items) > 0 and float(total_cost) <= float(Credit):
                        ...
                        def store_and_create_frames():
                            ...
                            for display_plbl in range(len(names_store_lbl)):
                                display_cart_items = customtkinter.CTkLabel(scrollable_receipt,
                                                                            ...
                                                                            text_color="#4a4a4a")
                                display_cart_items.grid(row=0 + display_plbl, column=0, padx=5, pady=1,
